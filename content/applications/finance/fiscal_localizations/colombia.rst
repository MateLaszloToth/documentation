...
.. _colombia/configuration/company:
...
- Configure the available :guilabel:`Certificates` to sign the electronic documents:
  ...
  - :guilabel:`Certificate`: upload the certificate file in PEM format. After uploading a valid
    certificate file, the following field displays:
...
Identification types, defined by the |DIAN|, are available on the :doc:`contact form
<../../essentials/contacts>`, as part of the Colombian localization. Colombian contacts **must**
...
The contact's responsibility codes (section 53 in the :abbr:`RUT (Registro único tributario)`
document) are included as part of the electronic invoicing module, as it is required by the |DIAN|.

The required fields can be found under :guilabel:`Sales & Purchase` tab in the :guilabel:`Fiscal
Information` section:
...
<colombia/dian-environment-config>`, instead of configuring these fields manually, click the
...
created from the sales order or manually generated. To create a new invoice, go to
...
After the :ref:`invoice confirmation <colombia/invoice-creation>`, click :guilabel:`Print & Send`.
In the wizard that appears, make sure to enable the :guilabel:`DIAN` and :guilabel:`Email`
checkboxes to send an XML to the |DIAN| web service and the validated invoice to the client fiscal
email. After that, click :guilabel:`Print & Send`:
...
- :guilabel:`Download`: To download the sent XML file, even if the |DIAN| result was
  rejected.
- :guilabel:`Fetch Attached Document`: To download the generated attached document file,
  even if the |DIAN|.
...
ID*, *Address*, *Products*, *Taxes*). In such cases, a validation error message is displayed and
sending is blocked.
